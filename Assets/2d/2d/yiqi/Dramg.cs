...
//给空间添加监听事件要实现的一些接口
public class Dramg : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler, 
IEndDragHandler, IPointerEnterHandler, IPointerExitHandler {

	public GameObject tie2;
	public GameObject tie5;

	public  GameObject mgimage;
	private Animation _mgfang;

	private Animation _animation;
	void Awake()
	{
		_animation = GetComponent<Animation>();
		_mgfang=mgimage.GetComponent<Animation>();
		mgimage.SetActive (false);
	}
	...
	Vector3 imgReduceScale = new Vector3(4.82f, 4.6f, 1);   //设置图片缩放
	Vector3 imgNormalScale = new Vector3(4.822f,4.64f, 1);   //正常大小
	...
	void OnTriggerStay2D(Collider2D other) {
		...
		_animation.Play("idel");
		StartCoroutine(WaitAndAnim());

	}
	IEnumerator WaitAndAnim(){
		yield return new WaitForSeconds(1.5f); 

		mgimage.SetActive (true);

		_mgfang.Play ("mgfangcheng");
	}
