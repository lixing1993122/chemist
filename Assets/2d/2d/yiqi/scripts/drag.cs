...
IEndDragHandler, IPointerEnterHandler, IPointerExitHandler {
	...
	//public GameObject jiujingd;
	//private Transform jiujingdspawnpoint;

	bool mousedown;
	private RectTransform imRect;  
	public RectTransform canvas;          //得到canvas的ugui坐标
	private RectTransform imgRect;        //得到图片的ugui坐标
	Vector2 offset = new Vector3();    //用来得到鼠标和图片的差值
	Vector3 imgReduceScale = new Vector3(0.7f, 0.9f, 1);   //设置图片缩放
	Vector3 imgNormalScale = new Vector3(0.8f,1f, 1);   //正常大小

	// Use this for initialization
	void Start () {
		//jiujingdspawnpoint = transform.Find("jiuposition");

		imgRect = GetComponent<RectTransform>();
		imRect = obj.GetComponent<RectTransform>();
	}

	//当鼠标按下时调用 接口对应  IPointerDownHandler
	public void OnPointerDown(PointerEventData eventData)
	{
		//Instantiate(jiujingd,jiujingdspawnpoint.position,jiujingdspawnpoint.rotation);
